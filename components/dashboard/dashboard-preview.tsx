  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-3xl">
        ...
        <div className="mt-4">
          <WidgetChart type={item.chartType} item={item} expanded />
        </div>
      </DialogContent>
    </Dialog>
  )
...
function WidgetChart({ type, item, expanded = false }: { type: string, item: AnalysisResult, expanded?: boolean }) {
  ...
  if (!item.parameters || !item.parameters.xAxis || !item.parameters.yAxis) {
    return <div className={`flex ${heightClass} items-center justify-center text-xs text-red-500 text-center px-4`}>No chart parameters generated.</div>
  }
